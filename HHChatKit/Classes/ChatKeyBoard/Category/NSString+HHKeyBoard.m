...
@implementation NSString (HHKeyBoard)
...
//        for (HHGroupEmojiModel *groupModel in HHChatKeyBoardManager.sharedManager.groupEmojis) {
//            if (groupModel.type != HHEmojiTypeSmall) {
//                continue;
//            }
//
//            for (HHEmojiModel *model in groupModel.emojis) {
...
+ (NSMutableAttributedString *)emotionImgsWithString:(NSString *)string {
    ...
    NSDictionary *attrDict = @{ NSFontAttributeName: [UIFont preferredFontForTextStyle:UIFontTextStyleBody]};
    NSMutableAttributedString * attStr = [[NSMutableAttributedString alloc]initWithString:string attributes:attrDict];
...
//                HHEmojiModel *model = groupModel.emojis[i];
...
//                        textAttachment.bounds = CGRectMake(0, -3, 22, 22);
